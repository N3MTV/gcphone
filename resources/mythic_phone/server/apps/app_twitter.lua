...
function getUser(id, cb)
  exports['ghmattimysql']:scalar("SELECT avatar_url as authorIcon FROM twitter_accounts WHERE twitter_accounts.id = @id", {
    ['id'] = id
  }, function (avatar)
    cb(avatar)
  end)
...
function TwitterPostTweet(char, message, cb)
  ...
  getUser(cData.id, function (user)
    exports['ghmattimysql']:execute("INSERT INTO twitter_tweets (`authorId`, `message`) VALUES(@authorId, @message);", {
      ['authorId'] = cData.id,
      ['message'] = message
    }, function (id)
...
function TwitterCreateAccount(id, avatarUrl, cb)
  exports['ghmattimysql']:execute('INSERT IGNORE INTO twitter_accounts (`id`, `avatar_url`) VALUES(@id, @avatarUrl)', {
    ['id'] = id,
    ['avatarUrl'] = avatarUrl
  }, cb)
end
...
AddEventHandler('mythic_phone:server:twitter_login', function()
  ...
  getUser(cData.id, function (user)
    if user == nil then
      ...
      TriggerClientEvent('mythic_phone:client:twitter_setAccount', cData.source, username, 'https://pbs.twimg.com/profile_images/1114565338346078214/HZSe7qxC_400x400.jpg')
    else
      TriggerClientEvent('mythic_notify:client:SendAlert', cData.source, { type = 'success', text = 'Logged Into Twitter'})
...
AddEventHandler('mythic_phone:server:twitter_setAvatarUrl', function(avatarUrl)
  local char = exports['mythic_base']:getPlayerFromId(source).getChar()
  local cData = char.getCharData()
  exports['ghmattimysql']:execute("UPDATE `twitter_accounts` SET `avatar_url`= @avatarUrl WHERE twitter_accounts.id = @id", {
    ['id'] = cData.id,
    ['avatarUrl'] = avatarUrl
  }, function (result)
    if (result == 1) then
